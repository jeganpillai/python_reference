# Question: Given a list of numbers create a new list that contains all the prime numbers from the previous list that are less than a given number n
# Video: https://www.youtube.com/watch?v=rMIcEQMOtEc 
...
# Approach 2: create a function to validate Primary number 
def Grow_With_Data(input_list, num):
    def is_prime(n):
        if n < 2 or n >= num:
            return False
...
# Approach 3: modify the code to List Comprehension 
def Grow_With_Data(input_list, num):
    def is_prime(n):
        for i in range(2, int(n**0.5)+1):
            if n % i == 0:
